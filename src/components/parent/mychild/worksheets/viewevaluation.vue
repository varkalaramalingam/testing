<template>
  <div>
    <div v-show="byeMsg">
      <h1><b style="color: green">Completed</b></h1>
    </div>
    <div class="col-lg-12">
    ...
    <div class="col-lg-12 worksheetstable reset-padding reset-margin">
        ...
        <div v-if="!show && selectedWorksheet.worksheet_type == 2">
          <div v-if="docUrl != null && docUrl != undefined">
            ...
            <div
              ...
              "
            >
              <img :src="docUrl" height="600px" width="700" />
            </div>
          </div>
          ...
          <div v-if="submittedDocUrls.length > 0">
            ...
            <a v-for="url in submittedDocUrls" :href="url" target="_blank">{{
              url
            }}</a>
        ...
        <div v-else-if="!show && selectedWorksheet.worksheet_type == 1">
          <div v-for="(question, index) in testQuestion">
            ...
            >
              ...
              <div
                style="float: right; margin-left: 100px; margin-bottom: 20px"
              >
                <input
                  disabled
                  type="radio"
                ...
                />
                <label disabled for="two">Partially Correct</label>
                <input
                  disabled
                  type="number"
      ...
      >
        <button
          ...
          @click="
            saveAndShowNextQuestion(null, false, 0)
          "
          class="assign-button"
        >
          Next
...
export default {
  ...
  methods: {
    ...
    getSubmittedDocs: function () {
      let ws_test_id = Number(this.selectedWorksheet.id);
    ...
    saveAndShowNextQuestion: function (question, passage, q_id) {
      ...
      this.documenturl = null;
      console.log("selected question index : "+ this.selectedQuestionIndex);
      console.log("passage question index : "+ this.passageQuestionIndex);
      console.log("selected option : "+ this.selectedOption);
    },
    ...
    submit: function (question) {
      ...
      if (this.selectedOption == 0) {
        subQueAnsObj.incorrect = true;
        subQueAnsObj.marks = 0;
      } else if (this.selectedOption == 1) {
        subQueAnsObj.correct = true;
        subQueAnsObj.marks = queObj.max_marks;
      } else if (this.selectedOption == 2) {
        ...
        subQueAnsObj.partiallycorrect = true;
        subQueAnsObj.marks = this.partialMarks;
      }

      //   UserService.updateEvaluation(this.tenantId, subQueAnsObj)
      //     .then((response) => {
      //       console.log("First this  has to be executed");
      //         let obj = {
      //         studentId: this.myChild.studentid,
      ...
      //             console.log(_.map(this.submittedQuestions ,function(data){return data.marks}))
      //             let  marks = _.reduce(_.map(this.submittedQuestions ,function(data){return data.marks}), function(data,num) { return data+num});

      //             console.log("total marks is : "+ marks);
      //             let updateObj = {
      //               id: this.myChild.id,
      //               ws_testid: this.myChild.ws_testid,
      //               studentid: this.myChild.studentid,
      //               submitteddate: this.myChild.submitteddate,
      //               securedmarks: marks,
      //               numberofattempt: this.myChild.numberofattempt,
      //               statusid: this.myChild.statusid
      //             }

      //               UserService.updateStudentMarksInWorksheet(this.tenantId,this.myChild.id,updateObj)
      //                 .then((response) => {
      //                   console.log("Second this  has to be executed");
      //                 }).catch(error => console.log(error));
      //          }).catch((error) => {console.log(error)})
      //     })
      //     .catch((error) => { console.log(error)});
    ...
    getSavedQuestionsAns: function () {
      ...
      UserService.getSubmittedQuestions(this.tenantId, obj)
        .then((response) => {
          this.submittedQuestions = response.data.resultsMap.questions;
          console.log(JSON.parse(JSON.stringify(this.submittedQuestions)));
    ...
    prepareQuestionObj: function (questionsData) {
      ...
      console.log("Questions Data is :" + JSON.stringify(questionsData));
    ...
    getCkFinalData: function (data) {
      ...
      if (temp.indexOf("</p>") != -1) {
        temp = temp.slice(0, temp.lastIndexOf("</p>"));
      }
      questionDesc = temp;
      return questionDesc;
    },
