...
<div class="container-fluid">
   <div class="row">
    <div class="page-layout">
        <div class="row">
            <div class="col-lg-6">
                <div class="page-heading"><span v-html="$t('yettoadmitstudents')"/></div>
...
<router-link to="/newstudent" data-target="#worksheetPopUp" tag="button" class="common-button pull-right">New Student</router-link>
        ...
        <hr/>
         ...
         <div style="float:left;width:25%" class="col-lg-4">
            ...
            <select class="form-control gradeFilter" @change="selectGrade()" style="width:51%;margin-right:7px" v-model="classdetails.gradeid">
                  ...
                  <option v-for="grade in gradeListCopy" v-bind:value="grade.id">{{grade.gradename}}</option>
            </select>
         </div>
         <div style="float:left;width:25%" class="col-lg-4">
            <label class="col-sm-1 control-label" style="width:35% !important">Status :</label>
            <select class="form-control gradeFilter" @change="selectStatus()" style="width:51%;margin-right:7px" v-model="classdetails.status">
                  <option v-bind:value=0>Yet To Admit</option>
                  <option v-bind:value=2>Rejected</option>
            ...
            <div class="divTable">
                <div class="divTableBody">
                    <div class="divTableRow">
                        ...
                        <div class="divTableCell">Change Status</div>
                        <div class="divTableCell" v-if="classdetails.status==0">Admit</div>
                    </div>
                    ...
                    <div class="divTableRowdata" v-for="student in paginatedData" :key="student.id">
                       <div class="divTableCelldata">{{student.id}}</div>
                        <div class="divTableCelldata"> 
                            <!-- <router-link to="/student" v-on:click.native="eachstudent(student)">s -->
                            {{student.name}}
                            <!-- </router-link> -->
                        </div>
                        ...
                        <div class="divTableCelldata">{{student.inactive}}</div>
                        ...
                        <div class="divTableCelldata" v-if="student.status==0"><button type="button" class="btn btn-danger" v-on:click="changestudentstatus(student,2)">Reject</button></div>
                        <div class="divTableCelldata" v-if="student.status==2"><button type="button" class="btn btn-info" v-on:click="changestudentstatus(student),0">Yet To Admit</button></div>
                        <div class="divTableCelldata" v-if="student.status==0"><button type="button" class="btn btn-success" v-on:click="setstudenttoclassroom(student)">Admit</button></div>
                    </div>
        ...
        <div class="modal fade" id="editstudent" role="dialog">
            ...
            <form name="editstudentmodal">
               ...
               <div class="row">
                  <div class=col-lg-6>
                     ...
                     <!-- <div class="row">
                        <div class="col-md-12">
                           <label><input type="radio"  value=true v-model="student.inactive">Inactive</label>
                           <label><input type="radio" value=false v-model="student.inactive">Active</label>
                        </div>
                     </div>
                     <br> -->
                  </div>
                     ...
                     <div class="row">
                        ...
                        <div class="col-md-8">
                           <select class="form-control form-input" v-model="student.gradename" paceholder="Select" v-if="student.sectionname==null" >
                              <option value="" disabled selected>Select your option</option>
                              <option v-for="grade in gradesbasedonboard" v-bind:value="grade.gradename">{{grade.gradename}}</option>
                           </select>
         ...
         <div class="modal fade" id="addstudenttoclassroom" role="dialog">
            <div class="modal-dialog">
               <div class="modal-content">
                  ...
                  <div class="modal-body flex-container">
                     <div>
                        <label class="col-sm-1 control-label" style="width:35% !important">Academic Year:</label>
                        <select  class="form-control gradeFilter" v-model="activeacademicDetails" v-on:change="changingacademicyearforaddingstudenttoclassroom()" style="width:30%" >
                              <option value="" disabled>-- Select Academic year --</option>
                              <option v-for="academic in activeacademicsandboards" 
                                 v-bind:value="{academicyearid: academic.id,syllabustype:academic.syllabustype}" 
                                 :key='academic.id+academic.syllabustype'
                     ...
                     <div>
                        <label class="col-sm-1 control-label" style="width:35% !important">Grade :</label>
                        <select class="form-control gradeFilter" @change="selectGradeforaddingstudenttoclassroom()" style="width:31%;margin-right:7px" v-model="gradeid">
                              <option v-for="grade in gradesList" v-bind:value="grade.id" :key="grade.id">{{grade.gradename}}</option>
                        </select>
                     </div>
                     <br>
                     <div>
                        <label class="col-sm-1 control-label" style="width:35% !important">Section :</label>
                        <select class="form-control gradeFilter" style="width:31%;margin-right:7px" v-model="classid" >
                              <!-- <option v-bind:value={classid:0,sectionname:emptystring}>All</option> -->
                              <option v-for="section in sectionsList" v-bind:value="section.id" :key="section.id">Section - {{section.sectionname}}</option>
                        </select>
                     </div>
                     <br>
                     <div>
                        <label class="col-sm-1 control-label" style="width:35% !important">Name :</label>
                        <label class="col-sm-1 control-label" style="width:35% !important;color:blue">{{singlestudent.name}}</label>
                     </div>
                  </div>
                  <br>
                  <div class="text-center">
                     <button id="btn-reset"  data-dismiss="modal" class="btn btn-info button"> Cancel</button>
                     <button id="btn-Register" type="submit" name="submit" v-on:click.prevent="addstudenttoclassroom()" class="btn btn-info button">Add</button>
                  </div>
                  <br>
               </div>
            </div>
...
//   "acdemicyearandboard"
  ...
  created() { 
    ...
    //      }
         ...
         this.getSectionsListforaddingstudenttoclassroom();
    //      this.getGradeList();
      })
  ...
  methods: {
   ...
   //  },
    ...
    //   this.classdetails.gradeid = this.classdetails.gradeid.toString();
    //   this.classdetails.status = this.classdetails.status.toString();
      UserService.yetToAdmitStudents(this.id, this.classdetails).then((response) => {
    ...
    getGradeList : function(){
      UserService.ClassroomGrades(this.academicDetails.syllabustype, this.id).then((response) => {
         ...
         console.log(JSON.stringify(response.data))
          this.classdetails.gradeid = this.gradeListCopy[0].id;
          this.gradeid = this.gradeListCopy[0].id;
   ...
   selectGrade: function(){
      this.gradeid = this.classdetails.gradeid;
      this.getStudentsList();
      this.selectGradeforaddingstudenttoclassroom();
   },
   selectStatus: function(){
       this.getStudentsList();
   },
   changestudentstatus: function(student, status){
       console.log(student)
       let obj = {
           "studentid" : student.id,
            "status" : status
       }
       UserService.changeStudentStatus(this.id, obj).then((response) => {
            console.log(response.data);
            this.getStudentsList();
        });
   },
   setstudenttoclassroom: function(student){
      this.singlestudent = student;
      this.popUp("addstudenttoclassroom", this.open);
      this.gradeid = this.classdetails.gradeid;
      this.selectGradeforaddingstudenttoclassroom();
   },
   addstudenttoclassroom: function(){
      let obj = {
         "id": this.classid,
         'admissionnumber':this.singlestudent.admissionnumber
      }
      console.log(obj);
      UserService.NewStudentToSection(obj,this.id).then((response)=>{
         console.log(JSON.stringify(response.data));
         this.getStudentsList();
         this.popUp("addstudenttoclassroom",this.close);
      })
   },
   changingacademicyearforaddingstudenttoclassroom: function(){
      this.academicDetails = this.activeacademicDetails;
      this.getSectionsListforaddingstudenttoclassroom();
   },
   getSectionsListforaddingstudenttoclassroom: function(){
      UserService.Sections(this.academicDetails,this.id).then((response)=>{
         console.log(JSON.stringify(response.data));
         this.gradesList = [];
         this.sectionsList=[];
         ...
         console.log(this.gradesList)
         for (let i = 0; i < sectionsLength; i++) {
            if (this.gradeid == this.sectionsListCopy[i].gradeid) {
               this.sectionsList.push(this.sectionsListCopy[i]);
            }
         }
         if(this.sectionsList.length > 0){
            this.classid = this.sectionsList[0].id;
         }
         console.log(this.classid);
      })
   },
   selectGradeforaddingstudenttoclassroom: function(){
      this.sectionsList = [];
   ...
   // getGradeListforaddingstudenttoclassroom : function(){
...
//    },
   selectSection: function(){
    ...
    selectGradebasedonBoard: function() {
      ...
      UserService.ClassroomGrades(boardforgrade, this.id).then(
        response => {
